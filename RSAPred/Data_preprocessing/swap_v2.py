...
if __name__ == "__main__":
    seeds = [0,1,2]
    ablations = ["target_swap","ligand_swap"]
    if len(sys.argv) != 3:
    ...
    unique_molecules = list(data_df['name'].unique())
    print(unique_rnas)
    ...
    for seed in seeds:
        ...
        for ablation in ablations:
            ...
            outfile = f"{sys.argv[2]}_{ablation}_seed{seed}.csv"
            ...
            if ablation=="target_swap":
                swapped_rna_ids = guaranteed_derangement(unique_rnas, seed)
                rna_swap_dict = dict(zip(unique_rnas, swapped_rna_ids))
                print("\nRNA rotation mapping:")
                for original, swapped in rna_swap_dict.items():
                    print(f"{original} -> {swapped}")
            elif ablation=="ligand_swap":
                swapped_mol_ids = guaranteed_derangement(unique_molecules, seed)
                mol_swap_dict = dict(zip(unique_molecules, swapped_mol_ids))
                print("\Mol rotation mapping:")
                for original, swapped in mol_swap_dict.items():
                    print(f"{original} -> {swapped}")

            # Create a list to store the new rows
            new_rows = []
            
            # Build new dataset with swapping
            ...
            for _, row in data_df.iterrows():
                try:
                    ...
                    original_mol = row['name']
                    if ablation=="target_swap":
                        swapped_rna = rna_swap_dict[original_rna]
                        swapped_rna_row = data_df[data_df['Target_RNA_ID'] == swapped_rna].iloc[0]
                    elif ablation=="ligand_swap":
                        swapped_mol = mol_swap_dict[original_mol]
                        swapped_mol_row = data_df[data_df['name'] == swapped_mol].iloc[0]
                    new_row = {}
                    for col in rna_cols:
                        if ablation=="target_swap":
                    ...
                    for col in mol_cols:
                        if ablation=="target_swap":
                            ...
                            new_row[col] = swapped_mol_row[col]
                    ...
                    #     'Original_Molecule': original_mol,
                    #     'Swapped_Molecule': swapped_mol,
                    #     'pKd': row['pKd']
                    # })
                    
                except Exception as e:
                    print(f"Error processing molecule {original_mol} with RNA {original_rna}: {str(e)}")
                    skipped_rows += 1
                    continue
