...
params = [4, 2, 128, 128]
model_path = "Model/Hariboss_Model_baseline.pth"

net = GerNA(params, trigonometry = True, rna_graph = False, coors = False, coors_3_bead = False, uncertainty=True)
...
with torch.no_grad():
    ...
    for batch_index, [batch_RNA_repre, batch_seq_mask, batch_Mol_Graph, batch_RNA_Graph, batch_RNA_feats, batch_RNA_C4_coors,batch_RNA_coors, batch_RNA_mask, batch_Mol_feats, batch_Mol_coors, batch_Mol_mask, batch_Mol_LAS] in enumerate(validDataLoader):
        ...
        _, interaction  = net( batch_RNA_repre, batch_seq_mask, batch_RNA_Graph, batch_Mol_Graph, batch_RNA_feats, batch_RNA_C4_coors, batch_RNA_coors, batch_RNA_mask, batch_Mol_feats, batch_Mol_coors, batch_Mol_mask, batch_Mol_LAS )        
        pairwise_list += interaction

target_binding = []
for i in range(len(pairwise_list)):
    max_values = np.max(pairwise_list[i].detach().cpu().numpy(), axis=1)
    target_binding.append(np.round(max_values, 4))

df = pd.DataFrame(np.array(target_binding).T)

df.columns = [f"mol{i+1}" for i in range(len(pairwise_list))]

csv_file = "RNA_binding.csv"
