...
def target_swap(dataset, seed=0):
    ...
    random.seed(seed)
    rna_ids = list(dataset.interaction_data["rna"].unique())
    swapped_rna_ids = guaranteed_derangement(rna_ids, seed)
    inds = dict(zip(rna_ids, swapped_rna_ids))
...
def ligand_swap(dataset, seed=0):
    ...
    dset_new = copy.deepcopy(dataset)
    random.seed(seed)
    mol_ids = list(dataset.interaction_data["mol"].unique())
    swapped_mol_ids = guaranteed_derangement(mol_ids, seed)
    inds = dict(zip(mol_ids, swapped_mol_ids))
    dset_new.interaction_data["mol"] = dset_new.interaction_data["mol"].apply(lambda i:inds[i])
    return dset_new
