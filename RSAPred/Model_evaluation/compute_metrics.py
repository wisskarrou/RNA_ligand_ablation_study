...
def compute_metrics(preds_path):
    ...
    df_global_metrics = df_inference.groupby(["ablation","seed"]).apply(custom_multi_agg).groupby(["ablation"]).agg(lambda x: f"${x.mean():.3f} \pm {x.std():.3f}$")
    df_target_wise_metrics = df_inference.groupby(["ablation","seed","RNA_id"]).apply(custom_multi_agg).add_suffix("_target").groupby(["ablation","seed"]).mean().groupby(["ablation"]).agg(lambda x: f"${x.mean():.3f} \pm {x.std():.3f}$")
