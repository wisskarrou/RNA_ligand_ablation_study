...
allowable_features = {
    ...
    #-------------------------------Supplementary--------------------------------#
    'possible_degree_supple_list' : [0, 1, 2, 3, 4, 5],
    'possible_ExplicitValence_list' : [1, 2, 3, 4, 5, 6],
    'possible_ImplicitValence_list' : [0, 1, 2, 3, 4, 5],
...
##########       MONN  partner        #############
elem_list = ['C', 'N', 'O', 'S', 'F', 'Si', 'P', 'Cl', 'Br', 'Mg', 'Na', 'Ca', 'Fe', 'As', 'Al', 'I', 'B', 'V', 'K', 'Tl', 'Yb', 'Sb', 'Sn', 'Ag', 'Pd', 'Co', 'Se', 'Ti', 'Zn', 'H', 'Li', 'Ge', 'Cu', 'Au', 'Ni', 'Cd', 'In', 'Mn', 'Zr', 'Cr', 'Pt', 'Hg', 'Pb', 'W', 'Ru', 'Nb', 'Re', 'Te', 'Rh', 'Tc', 'Ba', 'Bi', 'Hf', 'Mo', 'U', 'Sm', 'Os', 'Ir', 'Ce','Gd','Ga','Cs', 'unknown']#原子
aa_list = ['A', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'V', 'W', 'Y']#氨基酸
atom_fdim = len(elem_list) + 6 + 6 + 6 + 1
bond_fdim = 6
max_nb = 6


def MONN_onek_encoding_unk(x, allowable_set):
    if x not in allowable_set:
        x = allowable_set[-1]
    return list(map(lambda s: x == s, allowable_set))  #01编码


def MONN_atom_features(atom):
    return np.array(MONN_onek_encoding_unk(atom.GetSymbol(), elem_list) #one-hot encoding
            + MONN_onek_encoding_unk(atom.GetDegree(), [0,1,2,3,4,5]) 
            + MONN_onek_encoding_unk(atom.GetExplicitValence(), [1,2,3,4,5,6])
            + MONN_onek_encoding_unk(atom.GetImplicitValence(), [0,1,2,3,4,5])
            + [atom.GetIsAromatic()], dtype=np.float32)


def MONN_bond_features(bond):
    bt = bond.GetBondType()
...
atom_id = {'C': 1, 'N': 2, 'Br': 3, 'O': 4, 'S': 5, 'Cl': 6, 'F': 7, 'I': 8, 'As': 8, 'P': 8, 'K': 8, 'Na': 8, 'Ge': 8, 'Se': 8, 'Au': 8, 'Pt': 8,'Rh': 8, 'B': 8, 'H': 8, '*': 8, 'Fe': 8, 'unknown':8}
test_atom_list = [1,2,3,4,5,6,7,8]
def simplify_atom_to_onehot_feature_vector(atom):
    #features = onek_encoding_unk(atom_id[Chem.GetPeriodicTable().GetElementSymbol(atom.GetAtomicNum())], test_atom_list) + \
    features = onek_encoding_unk(atom_id.get(Chem.GetPeriodicTable().GetElementSymbol(atom.GetAtomicNum()), 8), test_atom_list) + \
           onek_encoding_unk(int(atom.GetChiralTag()), allowable_features['possible_chirality_list']) + \
           onek_encoding_unk(atom.GetTotalDegree(), allowable_features['possible_degree_list']) + \
           onek_encoding_unk(atom.GetFormalCharge(), allowable_features['possible_formal_charge_list']) + \
           onek_encoding_unk(int(atom.GetTotalNumHs()), allowable_features['possible_numH_list']) + \
...
def mol_to_graph_data_obj(mol):
    ...
    for atom in mol.GetAtoms():
        #atom_feature = atom_to_feature_vector(atom)   #change to ont-hot encoding
        atom_feature = atom_to_onehot_feature_vector(atom)
        #atom_feature = MONN_atom_features(atom)  #MONN atom feature
    ...
    if len(mol.GetBonds()) <= 0:  # mol has no bonds
        ...
        for bond in mol.GetBonds():
            ...
            #edge_feature = bond_to_feature_vector(bond)  #change to one-hot encoding for bond features
            edge_feature = bond_to_onehot_feature_vector(bond)
            #edge_feature = MONN_bond_features(bond)  #MONN bond feature
...
def simplify_atom_to_graph_data_obj(mol):
    atom_features_list = []
    for atom in mol.GetAtoms():
        atom_feature = simplify_atom_to_onehot_feature_vector(atom)
        atom_features_list.append(atom_feature)
    x = torch.tensor(np.array(atom_features_list), dtype=torch.long)
    if len(mol.GetBonds()) <= 0:  # mol has no bonds
        num_bond_features = len(get_bond_onehot_feature_dim())  # bond type & direction
        edge_index = torch.empty((2, 0), dtype=torch.long)
        edge_attr = torch.empty((0, num_bond_features), dtype=torch.long)
    else:
        edges_list = []
...
def mol_to_graph_data_robinRNA_obj(smile):
    
    mol = Chem.MolFromSmiles( smile )
    mol = Chem.AddHs(mol)
    
    smile_id = robin_smile_dict[smile]
    x = torch.tensor( robin_atom_reprs[smile_id] , dtype=torch.float)  #modify torch.long=>torch.float
    assert len(mol.GetAtoms())==len(robin_atom_reprs[smile_id] )
...
def mol_to_graph_data_robin_noRNA_obj(smile):
    mol = Chem.MolFromSmiles( smile )
    mol = Chem.AddHs(mol)
    smile_id = robin_noRNA_smile_dict[smile]
    x = torch.tensor( robin_noRNA_atom_reprs[smile_id] , dtype=torch.float)  #modify torch.long=>torch.float
    # bonds
    if len(mol.GetBonds()) <= 0:  # mol has no bonds
        num_bond_features = len(get_bond_feature_dims())  # bond type & direction
        edge_index = torch.empty((2, 0), dtype=torch.long)
    ...
    * :code:`a2a`: (Optional): A mapping from an atom index to neighboring atom indices.
    ...
    def __init__(self, mol_graphs: List[MolGraph], atom_messages: bool = False):
        ...
        for mol_graph in mol_graphs:
            ...
            self.a_scope.append((self.n_atoms, mol_graph.n_atoms))  #分段表示
